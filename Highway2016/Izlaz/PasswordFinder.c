...
BILL findPassword(char *pw) //pw-password
{
	...
	if ((fp = fopen("potvrde.txt", "r")) != NULL)
	{
		...
		while (fscanf(fp,"%s %s %s %s %d %d:%d:%d %d\n", &object.password, &object.checkpoint, &object.daychar,
			&object.month, &object.day, &object.hh, &object.min, &object.sek, &object.year) == 9)
		{
